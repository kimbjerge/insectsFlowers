...
def get_dataloader_sep_folder(data_dir: str,
    ...
    """
    #KBE??? data_transforms = transforms.Compose([transforms.ToTensor()])
...
def get_dataloader_single_folder(data_dir: str,
    ...
    #KBE??? data_transforms = transforms.Compose([transforms.ToTensor()])
    data_transforms = transforms.Compose([transforms.ToTensor(), 
                                          #transforms.Normalize(mean=[0.5], std=[0.5])]) # Imagenet
                                          transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])]) # Imagenet
    data_transformsGrey = transforms.Compose([transforms.ToTensor()])
    ...
    image_datasets = {
        x: SegmentationDataset(data_dir,
                               ...
                               subset=x,
                               transforms=data_transforms,
                               transformsGrey=data_transformsGrey)
        for x in ['Train', 'Test']
    }
    dataloaders = {
        x: DataLoader(image_datasets[x],
